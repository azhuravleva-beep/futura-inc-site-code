/* =====================================================================
   ...
   бухгалтерии 27.08: после кнопки шли четыре списка подряд, а карточки
   «Форматы работы» читатель видел только в самом низу страницы.
...
`;
  ...
  function build() {
    ...
    safe('широкий блок из блока 2', function () {
      ...
      // Куда ставить. По умолчанию — сразу за блоком 2, там же, где раздел
      // стоял в тексте. Но у типов «Споры» и «Сопровождение» блока «Что вы
      // получаете» в поле 1 нет, и на его месте зияет дыра: читатель после
      // кнопки упирается в четыре списка подряд, а карточки видит только
      // в самом низу. Решение Алисы 27.08 по предпросмотру бухгалтерии:
      // широкий блок занимает пустующий слот.
      //
      // Ставим после блока «Когда нужна…», а не после мид-CTA: мид-CTA
      // собирается ПОЗЖЕ этой функции и сам встаёт сразу за «Когда нужна…»,
      // то есть вклинивается между ними. Итог по чтению:
      // когда нужна → кнопка → широкий блок → текст блока 2.
      var результаты = document.querySelector('.s-built.is-s-results');
      var ситуации = document.querySelector('.s-built.is-s-cards');
      if (!результаты && ситуации) {
        ситуации.parentNode.insertBefore(сек, ситуации.nextSibling);
      } else {
