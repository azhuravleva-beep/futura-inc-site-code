 ...
 *   2. Дописывает в ссылки mailto тему письма — название страницы, поэтому
 *      входящее письмо само говорит, откуда его отправили. Только на страницах,
 *      где название звучит как нормальная тема письма («Ликвидация компании в
 *      ОАЭ»): услуги, решения, юрисдикции, страны, база знаний. На контактах,
 *      главной, страницах команды и в новостях тему не трогаем — там она
 *      выглядит нелепо и навязывает человеку, о чём он пишет.
 *
 *      ⚠️ В реестр лидов это НЕ попадает как источник: лид-машина кладёт тему
 *      в текст обращения, а колонки utm_source / page_url / referrer у писем
 *      остаются пустыми. Тему видит человек, который разбирает лид, — и сам
 *      отправитель, который волен её стереть.
 ...
 *   https://purge.jsdelivr.net/gh/azhuravleva-beep/futura-inc-site-code@main/contact-signals-1.0.1.js
 */
(function () {
  "use strict";
  ...
  /**
   * Разделы, где название страницы звучит как нормальная тема письма.
   * Локаль в пути не мешает: ищем сегмент, а не начало строки.
   */
  var SUBJECT_SECTIONS = [
    "/services/", "/solutions/", "/jurisdictions/", "/countries/", "/knowledge-base/"
  ];
  ...
  /**
   * Заголовок вкладки без брендового хвоста. Режем именно «… | Futura …»,
   * а не по первому разделителю: иначе «Гонконг — регистрация компании»
   * превратится в «Гонконг».
   */
  function pageName() {
    var t = String(document.title || "").trim();
    t = t.replace(/\s*[|—–\-]\s*Futura\b.*$/i, "").trim();
    if (!t) t = String(window.location.pathname || "/");
    return t.length > 70 ? t.slice(0, 70) + "…" : t;
  }

  /** Тему ставим не везде — см. пояснение в шапке. */
  function subjectAllowed() {
    var path = String(window.location.pathname || "/").toLowerCase();
    for (var i = 0; i < SUBJECT_SECTIONS.length; i++) {
      if (path.indexOf(SUBJECT_SECTIONS[i]) !== -1) return true;
    }
    return false;
  }
